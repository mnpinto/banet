...
__all__ = ['open_nc', 'crop', 'image2tiles', 'tiles2image', 'get_preds', 'predict_one', 'predict_time_old',
           'predict_time', 'predict_month', 'predict_nrt', 'split_mask']
...
def predict_time_old(path:InOutPath, times, weight_files:list, region:Region,
...
def predict_time(path:InOutPath, times, weight_files:list, region:Region,
                 threshold=0.05, save=True, max_size=2000, buffer=128,
                 product='VIIRS750', output='data', verbose=True):
    tstart, tend = times.min(), times.max()
    tstart = pd.Timestamp(f'{tstart.year}-{tstart.month}-01')
    tend = pd.Timestamp(f'{tend.year}-{tend.month}-01')
    ptimes = pd.date_range(tstart, tend, freq='MS')[1:-1]
    ...
    for time in progress_bar(ptimes):
        ...
        file_save = path.dst/f'{output}_{region.name}{tstr}.mat'
        if not file_save.is_file():
            ...
            for i, split in enumerate(si):
                ...
                del preds
                gc.collect()
            preds_full = np.zeros((len(preds_all[0]), *region.shape), dtype=np.float16)
            for i, split_idx in enumerate(si):
                preds_full[:, split_idx[0]:split_idx[1], split_idx[2]:split_idx[3]] = preds_all[i]
            sio.savemat(file_save, {'preds': preds_full}, do_compression=True)
            del preds_all, preds_full
            gc.collect()
    ...
    for time in progress_bar(ptimes):
        tstr = time.strftime('%Y%m')
        file = path.dst/f'{output}_{region.name}{tstr}.mat'
        data = sio.loadmat(file)['preds']
