...
__all__ = ['VIIRS375_download', 'VIIRS750_download', 'BandsFilter', 'BandsRename', 'MergeTiles', 'BandsAssertShape',
           'ActiveFires', 'ActiveFiresLog', 'InterpolateAngles', 'MirCalc', 'BaseDataset', 'ViirsDataset',
           'Viirs375Dataset', 'Viirs750Dataset', 'MCD64Dataset', 'FireCCI51Dataset', 'AusCoverDataset', 'MTBSDataset',
    ...
    def __call__(self, data:dict, *args, **kwargs):
        for b in self.bands:
            data[b] = [downsample(data[b][i], src_tfm=self.R_base.transform,
                                 dst_tfm=self.R_interp.transform,
                                 dst_shape=self.R_interp.shape,
                                 resampling='bilinear')
    ...
    def open_netcdf4(self, files:list) -> dict:
        ...
        for s in self.bands:
            ...
            for f0 in f:
                ...
                data = hdf_data[s][:].data.astype(np.float32)
                data[data <= -999] = np.nan
                data[data >= 65527] = np.nan
