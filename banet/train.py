...
class BCE(Module):
    ...
    def forward(self, x, y):
        ...
        return 50*bce(x.view(x.size()[0],-1),y.view(y.size()[0], -1)) #use 100x for 64 days model and 50x for 32 days model
...
def train_model(val_year, r_fold, path, model_path, n_epochs=8, lr=1e-2, nburned=10, n_episodes_train=2000,
        ...
        get_learner=False, save_to='banet-val-fold-v2.pth'):
    path_img = path/'images'
    train_files = pd.array(sorted([f.name for f in path_img.iterdir()]))
    ...
    info_train = set_info_df(dsets.train.items, satellite='VIIRS375', target_product='MCD64A1C6')
    info_valid = set_info_df(dsets.valid.items, satellite='VIIRS375', target_product='MCD64A1C6')

### debug
#    cand = info_train[info_train.ba > nburned]
#    print("v2 info_train: count:", len(info_train))
#    print("v2 ba>0:", (info_train.ba > 0).sum())
#    print("v2 ba>nburned:", len(cand))
#    print("v2 max ba:", info_train.ba.max())
###

    bs = sequence_len * n_sequences

    train_dl = CustomLoader(dsets.train,
                        sampler=SampleEpisode(dsets.train[0], n_episodes=2000,
    ...
    valid_dl = CustomLoader(dsets.valid,
                        sampler=SampleEpisode(dsets.valid[0], n_episodes=100,
                                              sequence_len=sequence_len, n_sequences=n_sequences,
    ...
    if do_cutout:
        dls.train.after_batch = Pipeline([
            ...
            Brightness(max_lighting=0.1, p=0.75),  # Paper: 0.4-0.6 range
